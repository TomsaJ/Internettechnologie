...
abstract class Product implements ActiveRecordInterface
{
     ...
     * @var string
     */
    public const TABLE_MAP = '\\Map\\ProductTableMap';
     ...
     * @phpstan-var ObjectCollection&\Traversable<ChildCategory> Cross Collection to store aggregation of ChildCategory objects.
     */
    ...
    public function initProductCatalogies(bool $overrideExisting = true): void
    {
        ...
        $this->collProductCatalogies->setModel('\ProductCatalogy');
    }
        ...
        if (null === $this->collProductCatalogies || null !== $criteria || $partial) {
            if ($this->isNew()) {
                ...
                if (null === $this->collProductCatalogies) {
                    ...
                    $collProductCatalogies->setModel('\ProductCatalogy');
     ...
     * to the current object by way of the product_catalogy cross-reference table.
     *
     * If the $criteria is not null, it is used to always fetch the results from the database.
     * Otherwise the results are fetched from the database the first time, then cached.
     * Next time the same method is called without $criteria, the cached collection is returned.
     ...
     */
    public function countCategories(?Criteria $criteria = null, $distinct = false, ?ConnectionInterface $con = null): int
    {
        $partial = $this->collCategoriesPartial && !$this->isNew();
        if (null === $this->collCategories || null !== $criteria || $partial) {
     ...
     * through the product_catalogy cross reference table.
     *
    ...
    {
        $productCatalogy = new ChildProductCatalogy();

        $productCatalogy->setCategory($category);
    ...
    public function removeCategory(ChildCategory $category)
    {
        if ($this->getCategories()->contains($category)) {
            ...
            $productCatalogy->setProduct($this);
            $this->removeProductCatalogy(clone $productCatalogy);
            $productCatalogy->clear();
